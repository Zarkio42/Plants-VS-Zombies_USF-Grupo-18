...
interface RawCardData {
  ...
  description: string;
  image: string;
  // Plantas: Variações para Custo e Recarga
  cost?: number; 
  'Sun cost'?: number; 
  recharge?: string; 
  Recharge?: string; 
  'sun-production'?: string;
  // Zumbis/Comuns: Variações para Resistência e Velocidade
...
interface PlantCard {
  ...
  image: string;
  cost: number; 
  recharge: string; // Tipo string, pode ser "Muito Rápida", "Lenta", etc.
  'sun-production': string; 
  area: string;
  usage: string;
  special: string;
...
type Card = PlantCard | ZombieCard;
...
const CardDetailsModal: FC<CardDetailsModalProps> = ({ card, onClose, getImageUrl }) => {
    const styles = card.type === 'Zombie' 
        ? { primary: 'bg-red-700', secondary: 'text-red-400', border: 'border-red-600' }
        : { primary: 'bg-green-700', secondary: 'text-green-400', border: 'border-green-600' };

    const PlantDetails = card.type === 'Plant' ? card as PlantCard : null;
    const ZombieDetails = card.type === 'Zombie' ? card as ZombieCard : null;

    const renderAttribute = (label: string, value: string | number | undefined) => {
        if (value === undefined || value === 'N/A' || value === 'PLANTA' || value === 0) return null;
        return (
            <div className="flex justify-between py-2 border-b border-gray-700/50">
                <span className="text-gray-400 font-semibold">{label}</span>
                <span className={`${styles.secondary} font-bold text-right`}>{value}</span>
            </div>
        );
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={onClose}>
            ...
            >
                {/* Cabeçalho */}
                <div className={`relative ${styles.primary} rounded-t-lg p-6 flex justify-between items-start border-b-2 ${styles.border}`}>
                ...
                <div className="p-6">
                    <div className="flex items-center space-x-6 mb-6">
                        ...
                        <div className="flex-grow">
                            <h3 className="text-xl font-bold text-gray-200 mb-2">Estatísticas</h3>
                            <div className="grid grid-cols-2 gap-4">
                                {/* Custo / Toughness */}
                                <div className="flex items-center space-x-2">
                                    {ZombieDetails ? <Shield className="w-5 h-5 text-red-500"/> : <Sun className="w-5 h-5 text-yellow-400"/>}
                                    <div>
                                        <p className="text-gray-500 text-xs">{ZombieDetails ? 'Resistência' : 'Custo Solar'}</p>
                                        <p className={`font-bold ${styles.secondary}`}>{ZombieDetails ? ZombieDetails.toughness : PlantDetails?.cost}</p>
                                    </div>
                                </div>
                                {/* Recarga / Velocidade */}
                                <div className="flex items-center space-x-2">
                                    {ZombieDetails ? <FastForward className="w-5 h-5 text-red-500"/> : <Clock className="w-5 h-5 text-blue-400"/>}
                                    <div>
                                        <p className="text-gray-500 text-xs">{ZombieDetails ? 'Velocidade' : 'Recarga'}</p>
                                        <p className={`font-bold ${styles.secondary}`}>{ZombieDetails ? ZombieDetails.speed : PlantDetails?.recharge}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <h3 className="text-xl font-bold text-gray-200 mb-2 mt-4">Atributos Extras</h3>
                    <div className="space-y-1">
                        ...
                        {!(PlantDetails || ZombieDetails) && <p className="text-gray-500">Nenhuma informação adicional disponível.</p>}
                    </div>
                </div>
...
export default function PvZDeckHome() { 
  ...
  const [sortBy, setSortBy] = useState<'name-asc' | 'name-desc' | 'cost-asc' | 'cost-desc' | 'recharge-asc' | 'recharge-desc' | 'plant-first' | 'zombie-first'>('name-asc');
  // ---------------------------------------------
  ...
  const fetchCardDetails = async (type: 'plants' | 'zombies', name: string): Promise<Card | null> => {
    const detailEndpoint = type === 'plants' 
        ? `${BASE_API_URL}/plants/${name}` 
        : `${BASE_API_URL}/zombies/${name}`;
    
    try {
      const response = await fetch(detailEndpoint);
      if (!response.ok) {
        console.warn(`Falha ao buscar ${type}: ${name}. Status: ${response.status}`);
        return null; 
      }
      
      const rawData: RawCardData = await response.json();
      
      if (type === 'plants') {
        
        const finalCost = rawData['Sun cost'] !== undefined 
            ? rawData['Sun cost'] 
            : rawData.cost !== undefined 
            ? rawData.cost 
            : 0;
        ...
        return {
          ...
          'sun-production': rawData['sun-production'] || 'PLANTA', 
          area: rawData.area || 'N/A',
          usage: rawData.usage || 'N/A',
          special: rawData.special || 'N/A',
          range: rawData.range || 'N/A',
          family: rawData.family || 'N/A',
        } as PlantCard;
        
      } else { // Zumbis
        
        const finalToughness = rawData.Toughness?.toString() || rawData.toughness?.toString() || 'N/A';
        const finalSpeed = rawData.Speed || rawData.speed || 'N/A';
        ...
        return {
          ...
          speed: finalSpeed, 
          stamina: rawData.stamina || 'N/A',
          weakness: rawData.weakness || 'N/A',
        } as ZombieCard;
      }

    } catch (error: unknown) { 
      if (error instanceof Error) {
        console.error(`Erro ao processar ${type} ${name}:`, error.message);
      } 
      return null;
    }
  };
  ...
  const fetchCards = useCallback(async () => {
    try {
      ...
      const [plantsListResponse, zombiesListResponse] = await Promise.all([
        fetch(`${BASE_API_URL}/plants`), 
        fetch(`${BASE_API_URL}/zombies`) 
      ]);

      if (!plantsListResponse.ok || !zombiesListResponse.ok) {
        throw new Error('Falha ao carregar as listas de nomes.');
      }

      const plantNames: string[] = (await plantsListResponse.json()) as string[];
      const zombieNames: string[] = (await zombiesListResponse.json()) as string[];
      
      const plantDetailPromises = plantNames.map(name => fetchCardDetails('plants', name));
      const zombieDetailPromises = zombieNames.map(name => fetchCardDetails('zombies', name));
      
      const detailedPlants = (await Promise.all(plantDetailPromises)).filter(c => c !== null) as Card[];
      const detailedZombies = (await Promise.all(zombieDetailPromises)).filter(c => c !== null) as Card[];
      
      const allCards = [...detailedPlants, ...detailedZombies];
      setCards(allCards); 
  ...
  const isCardComplete = (card: Card): boolean => {
    if (card.type === 'Plant') {
      ...
      return (
        ...
        zombieCard.speed !== 'N/A'
      );
    }
  };
  
  // Função para mapear Recarga (string) para um valor numérico para classificação
  ...
  const processedCards = useMemo(() => {
    ...
    sortableCards.sort((a, b) => {
        ...
        // --- NOVAS REGRAS DE AGRUPAMENTO (Plantas Primeiro / Zumbis Primeiro) ---
        if (sortBy === 'plant-first' || sortBy === 'zombie-first') {
            const isA_Plant = a.type === 'Plant';
            const isB_Plant = b.type === 'Plant';
            
            let typeComparison = 0; // 0 significa que são do mesmo tipo (ou a regra não se aplica)

            if (isA_Plant && !isB_Plant) {
                // A é planta, B é zumbi
                typeComparison = sortBy === 'plant-first' ? -1 : 1; 
            } else if (!isA_Plant && isB_Plant) {
                // A é zumbi, B é planta
                typeComparison = sortBy === 'plant-first' ? 1 : -1; 
            }
            
            // Se o tipo for diferente, retorna a ordem do tipo
            if (typeComparison !== 0) return typeComparison;

            // Se o tipo for o mesmo, usa o nome (A-Z) como desempate
        ...
        if (sortBy === 'cost-asc' || sortBy === 'cost-desc') {
            ...
            // Move Zombies (ou não-Plants) para o fim da lista quando o custo é o critério
            if (costA === Infinity && costB === Infinity) return a.name.localeCompare(b.name); 
        ...
        // Classificação por Recarga (aplica-se apenas a Plants)
        if (sortBy === 'recharge-asc' || sortBy === 'recharge-desc') {
             
            // Move Zombies (ou não-Plants) para o fim da lista
            if (a.type !== 'Plant' && b.type !== 'Plant') return a.name.localeCompare(b.name);
        ...
        <div className="bg-gradient-to-r from-gray-800/50 to-gray-900/50 backdrop-blur-sm rounded-2xl border border-gray-700 p-8 mb-8">
          <div className="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 md:space-x-4">
            ...
            <div className="w-full max-w-[10rem]">
                ...
                <select
                    ...
                    name="filterType"
                    value={filterType}
                    onChange={(e) => setFilterType(e.target.value as 'All' | 'Plant' | 'Zombie')}
            ...
            {/* 3. Classificar Por (COM NOVAS OPÇÕES) */}
            <div className="w-full max-w-[14rem]">
                <label htmlFor="sortBy" className="sr-only">Classificar por</label>
                <select
                    id="sortBy"
                    name="sortBy"
                    value={sortBy}
                    onChange={(e) => setSortBy(e.target.value as 'name-asc' | 'name-desc' | 'cost-asc' | 'cost-desc' | 'recharge-asc' | 'recharge-desc' | 'plant-first' | 'zombie-first')} 
                    className="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-green-500 appearance-none cursor-pointer transition-colors"
                >
          ...
          <div className="flex justify-center md:justify-end space-x-4 mt-6">
              <div className="bg-gray-900/80 border border-green-500/30 px-6 py-3 rounded-lg text-center">
                <p className="text-green-400 font-bold text-2xl">{filteredCards.length}</p>
              ...
              <div className="bg-gray-900/80 border border-purple-500/30 px-6 py-3 rounded-lg text-center">
                <p className="text-purple-400 font-bold text-2xl">{cards.length}</p>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20">
            <div className="text-center">
              ...
              <p className="text-gray-400 text-lg mt-4">Carregando cartas...</p>
            </div>
          </div>
        ) : filteredCards.length === 0 ? (
          <div className="text-center py-20 bg-gray-800/50 rounded-xl border border-gray-700">
            <p className="text-2xl font-bold text-red-500 mb-2">🚫 Nenhuma Carta Encontrada</p>
            <p className="text-sm text-yellow-500 mt-4">
                Verifique se a API ({BASE_API_URL}) está online ou ajuste seus filtros/busca.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4 gap-8">
            {filteredCards.map((card) => {
              ...
              const cardName = card.name?.toUpperCase() || 'CARTA DESCONHECIDA';
              const plantCard = card.type === 'Plant' ? (card as PlantCard) : null;
              const zombieCard = card.type === 'Zombie' ? (card as ZombieCard) : null;
              ...
              >
                ...
                {/* 1. TOPO: IMAGEM + NOME + BADGE */}
                <div className="relative pt-6 pb-2 px-4 flex flex-col items-center justify-center bg-gray-800/80">
                    
                    <div className="absolute top-4 right-4 z-10">
                        <span className={`${styles.badgeBg} px-3 py-1 rounded-full text-white text-xs font-bold shadow-md uppercase tracking-wider flex items-center space-x-1`}>
                            <styles.icon className="w-3 h-3"/>
                            <span>{plantCard ? plantCard['sun-production'] : styles.title}</span>
                        </span>
                    </div>
                    ...
                    {card.image ? (
                        <img 
                            ...
                            onError={(e: SyntheticEvent<HTMLImageElement, Event>) => {
                                ...
                                e.currentTarget.parentElement!.innerHTML = `<div class="h-32 flex items-center justify-center text-7xl ${styles.iconColor}">${card.type === 'Zombie' ? '💀' : '🌿'}</div>`;
                            }}
                        />
                    ) : (
                        <div className={`h-32 flex items-center justify-center text-7xl ${styles.iconColor}`}>{card.type === 'Zombie' ? '💀' : '🌿'}</div>
                    )}
                    ...
                    <div className="flex justify-between items-center mb-4 bg-gray-800 rounded-lg p-3 border border-gray-700">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <div className="text-left">
                                <p className="text-gray-500 text-xs">{zombieCard ? 'Toughness' : 'Custo'}</p>
                                <p className={`font-extrabold text-lg ${zombieCard ? 'text-red-400' : 'text-yellow-400'}`}>
                                    {zombieCard ? zombieCard.toughness : plantCard?.cost || 'N/A'}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <div className="text-left">
                                <p className="text-gray-500 text-xs">{zombieCard ? 'Velocidade' : 'Recarga'}</p>
                                <p className={`font-bold text-lg truncate ${zombieCard ? 'text-red-400' : 'text-blue-400'}`}>
                                    {zombieCard ? zombieCard.speed : plantCard?.recharge || 'N/A'}
                                </p>
                            </div>
                        </div>
                    </div>
      ...
      <div className="mt-12 bg-gradient-to-r from-gray-800/80 to-gray-900/80 backdrop-blur-sm rounded-2xl border border-gray-700 p-10 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-gray-400 mb-6">
            Selecione suas 16 plantas favoritas e crie estratégias imbatíveis contra os zumbis!
          </p>
          <button
            onClick={handleCreateDeck}
      ...
      <footer className="relative border-t border-gray-800 bg-black/50 py-6 mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-gray-500 text-sm">
            © 2025 Lawn of the Dead - Deck Builder - Projeto Universitário USF | Grupo 18
          </p>
